...
-- =========================================================
-- JOKJUNG BACK OFFICE - COST FIX CENTER V1.1
...
declare
    ...
    v_captured_cogs numeric := 0;
    v_zero_cost_lines bigint := 0;
...
begin
    ...
    -- Business reports use Thailand local calendar dates. Convert the local
    -- midnight boundaries to timestamptz explicitly so SQL session timezone
    -- cannot shift the requested period.
    v_from_ts := p_date_from::timestamp at time zone 'Asia/Bangkok';
    v_to_ts := (p_date_to + 1)::timestamp at time zone 'Asia/Bangkok';

    select
        round(coalesce(sum(coalesce(si.total_price,coalesce(si.unit_price,0)*coalesce(si.quantity,0))),0),2),
        round(coalesce(sum(coalesce(si.unit_cost,0)*coalesce(si.quantity,0)),0),4),
        count(*) filter(where coalesce(si.unit_cost,0) <= 0)
    into v_total_sales,v_captured_cogs,v_zero_cost_lines
    from public.sale_items si
    join public.sales s on s.id=si.sale_id
    where s.branch_id=v_branch
      and s.created_at >= v_from_ts
      and s.created_at < v_to_ts
    ...
    return jsonb_build_object(
        ...
        'captured_cogs', v_captured_cogs,
        'zero_cost_line_count', v_zero_cost_lines,
        'diagnostic_status', case
            when v_total_sales > 0 and v_captured_cogs <= 0 and v_total = 0 then 'COGS_ZERO_UNEXPLAINED'
            when v_total > 0 then 'ISSUES_FOUND'
            else 'OK'
        end,
        'items', v_items
    );
